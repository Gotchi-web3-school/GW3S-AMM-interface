...
export const fetchPoolBalances = async(pool: IPool, userAdress: string, contract: any): Promise<{balance: TokenAmount, amountA: TokenAmount, amountB: TokenAmount, share: Percent}> => {
    const poolToken = contract.ERC20.attach(pool.pair.liquidityToken.address)
    const pairContract = contract.pair.attach(pool.pair.liquidityToken.address);
    const reserves = await pairContract.getReserves()
    const totalSupply = await new TokenAmount(pool.pair.liquidityToken, await poolToken.totalSupply())
    const balance = await new TokenAmount(pool.pair.liquidityToken, await poolToken.balanceOf(userAdress));
