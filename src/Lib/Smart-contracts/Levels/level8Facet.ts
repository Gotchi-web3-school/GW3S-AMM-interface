...
import { TokenList, level8List } from "../../../Constants/list";
import { Pool } from "../../../Models/index";
import { interfaces } from "../../../Constants/interfaces";
import { Token } from "gotchiw3s-sdk";

export const start_l8 = async(tx: InitTx) => {
    try {    
        console.warn("INIT")
        console.log("///////////////////////////////////////////////")
        console.log("Level: " + 8)
        ...
        const gas = await tx.Facet?.estimateGas.initLevel8() 
        console.log("Gas cost: " + (ethers.utils.formatEther(gas?.toString() ?? "") + " MATIC"))
        
        const transaction = await tx.Facet?.initLevel8()
        ...
        tx.dispatch({type: "INIT", payload: 8})
...
export const completeL8 = async(tx: CompleteTx) => {
    try {    
        ...
        const gas = await tx.Facet?.estimateGas.completeL8()
        console.log("Gas cost: " + (ethers.utils.formatEther(gas?.toString() ?? "") + " MATIC"))
        
        const transaction = await tx.Facet?.completeL8()
        ...
        tx.toast({
            ...
            const hasCompleted = await tx.LoupeFacet?.hasCompletedLevel(tx.signer.account, 8)
...
export const openL8Chest = async(tx: ClaimTx): Promise<Array<Reward | undefined>> => {
    ...
    try {   
        ...
        //Estimation of the gas cost
        const gas = await tx.Facet?.estimateGas.openL8Chest()
        console.log("Gas cost: " + (ethers.utils.formatEther(gas?.toString() ?? "") + " MATIC"))

        const chestOpenned = await tx.Facet?.callStatic.openL8Chest()
        const transaction = await tx.Facet?.openL8Chest()
    
        tx.toast({
            title: `Open chest level 8`,
            description: `transaction pending at: ${transaction?.hash}`,
            position: "top-right",
            status: "warning",
            isClosable: true,
...
export const fetchAMMState = async(signer: any, instanceAddress: string, factory: string): Promise<{
    ...
    let initCode = '';
    let list: TokenList[] = level8List
    let pools: Pool[] = []
    try {
        const LeveL8Instance = new ethers.Contract(instanceAddress, interfaces.ILevel8Instance, signer.library)
        const Factory = new ethers.Contract(factory, interfaces.IFactory, signer.library?.getSigner(signer.account) ?? signer.library)
    
        for(let i = 0; i < 2; i++)
            list[i].address = await LeveL8Instance.tokens(i)
