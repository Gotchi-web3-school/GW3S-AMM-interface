...
import { TokenList, level12List } from "../../../Constants/list";
import { Pool } from "../../../Models/index";
import { interfaces } from "../../../Constants/interfaces";
import { Token } from "gotchiw3s-sdk";

export const start_l12 = async(tx: InitTx) => {
    try {    
        console.warn("INIT")
        console.log("///////////////////////////////////////////////")
        ...
        const gas = await tx.Facet?.estimateGas.initLevel12() 
        console.log("Gas cost: " + (ethers.utils.formatEther(gas?.toString() ?? "") + " MATIC"))
        
        const transaction = await tx.Facet?.initLevel12()
        ...
        tx.dispatch({type: "INIT", payload: 12})
...
export const completeL12 = async(tx: CompleteTx) => {
    try {    
        ...
        const gas = await tx.Facet?.estimateGas.completeL12()
        console.log("Gas cost: " + (ethers.utils.formatEther(gas?.toString() ?? "") + " MATIC"))
        
        const transaction = await tx.Facet?.completeL12()
        
        tx.toast({
            title: `Complete level 12`,
            description: `transaction pending at: ${transaction?.hash}`,
            position: "top-right",
            status: "warning",
            isClosable: true,
        ...
        tx.toast({
            ...
            const hasCompleted = await tx.LoupeFacet?.hasCompletedLevel(tx.signer.account, 12)
...
export const openL12Chest = async(tx: ClaimTx): Promise<Array<Reward | undefined>> => {
    ...
    try {   
        ...
        //Estimation of the gas cost
        const gas = await tx.Facet?.estimateGas.openL12Chest()
        console.log("Gas cost: " + (ethers.utils.formatEther(gas?.toString() ?? "") + " MATIC"))

        const chestOpenned = await tx.Facet?.callStatic.openL12Chest()
        const transaction = await tx.Facet?.openL12Chest()
    
        tx.toast({
            title: `Open chest level 12`,
            description: `transaction pending at: ${transaction?.hash}`,
            position: "top-right",
            status: "warning",
            isClosable: true,
...
export const fetchAMMState = async(signer: any, instanceAddress: string, levelLoupe: ethers.Contract, factoryId: number): Promise<{
    factory: string,
    initCode: string,
    list: TokenList[],
    pools: Pool[] 
    ...
    let factory: string = '';
    let initCode = '';
    let list: TokenList[] = level12List
    let pools: Pool[] = []
    try {
        factory = await levelLoupe.getFactoryLevel(12, factoryId);
        const LeveL12Instance = new ethers.Contract(instanceAddress, interfaces.ILevel12Instance, signer.library)
        const Factory = new ethers.Contract(factory, interfaces.IFactory, signer.library?.getSigner(signer.account) ?? signer.library)
    
        for(let i = 0; i < 2; i++)
            list[i].address = await LeveL12Instance.tokens(i)
