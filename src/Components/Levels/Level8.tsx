...
import { fetchAMMState } from "../../Lib/Smart-contracts/Levels/level8Facet"
...
const Level8: React.FC = () => {
    ...
    useEffect(() => {
        if (running === 8 && instanceAddress !== GlobalConst.addresses.ZERO_ADDRESS && factories) {
    ...
    return (
        ...
        <Stack direction={"row"} m="5rem" align="center">
            <Spacer />
                {amm?.pools && running === 8 && <AMM id={0} factory={amm.factory} initCode={amm.initCode} tokenList={amm.list} pools={amm.pools} />}
            <Spacer />
            <Card level={levels[8]}/>
        </Stack>
    </Box>
