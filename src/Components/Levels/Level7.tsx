...
import { fetchAMMState } from "../../Lib/Smart-contracts/Levels/level7Facet"
...
const Level7: React.FC = () => {
    ...
    useEffect(() => {
        if (running === 7 && instanceAddress !== GlobalConst.addresses.ZERO_ADDRESS && factories) {
    ...
    return (
        ...
        <Stack direction={"row"} m="5rem" align="center">
            <Spacer />
                {amm?.pools && running === 7 && <AMM id={0} factory={amm.factory} initCode={amm.initCode} tokenList={amm.list} pools={amm.pools} />}
            <Spacer />
            <Card level={levels[7]}/>
        </Stack>
    </Box>
